...
This plugin owns the `migrations` AgentDB namespace (kebab-case is implicit when the plugin name is the intent â€” same documented exception as `federation`). Follows the convention from ruflo-agentdb namespace convention (original ruflo repo). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
> **Routing note:** Earlier versions of these skills used `agentdb_hierarchical-*` and `agentdb_pattern-store` with namespace arguments â€” those tool families route by tier/ReasoningBank and ignore namespace strings. ADR-0001 fixed the skills to use `memory_*` for namespaced reads/writes.
...
- [`ADR-0001` â€” ruflo-migrations plugin contract (namespace-routing fix, smoke as contract)](./docs/adrs/0001-migrations-contract.md)
